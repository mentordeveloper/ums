 ...
 */
var show_overlay = '';
$(document).live('click', '.' + show_overlay, function() {
    // alert("sa");
...
//    $(document).live('click', '#TB_overlay', function() {
//        // alert("sa");
...
$("#activity_feed_submit").click(function() {
    var user_login_check = $("#user_login_check").val();
    if (user_login_check != '') {
        $(".activity_feed_form_share_process").css("display", "block");
...
//            alert(status);
            if (action == 'user_status' && (status == "What's on your mind?" || status == "")) {//alert(action);
                ...
                var request = $.ajax({
                    url: $("#base_url").val() + "event/feeds?ajax=true" + values + "&id=" + $("#event_id_hidden").val(),
                    type: "GET",
                    data: {feeds: values},
                    dataType: "html"
                });
        ...
        $.fancybox({
                            ...
                            'transitionOut': 'fade',
                            'type': 'iframe',
                            //'href': $('#base_url').val() + "instructor/load_login/?url=" + document.URL+ "&side=adminside"+ "&side_popup=adminside&k=" + $('#user_side_popup').val() ,
                            'href': $('#base_url').val() + "instructor/load_login/?url=" + document.URL+ "&side="+ $('#user_side_popup').val()+ "&side_popup=" + $('#user_side_popup').val() ,
                           // 'href': $('#base_url').val() + "login/?url=" + document.URL,
                        });
    }
...
function ajaxFileUpload()
{
    ...
    var ajax_req = $.ajaxFileUpload({
        url: $("#base_url").val() + "event/photo_file" + "?id=" + $("#event_id_hidden").val(),
        secureuri: false,
        fileElementId: 'image',
        dataType: 'json',
        success: function(msg) {
...
$(document).live('click', '.js_comment_feed_textarea', function() {
...
//$(document).live('click', '.comment_mini_button', function() {
    ...
    if (area_val == '') {
        ...
        var values = getForm(form_name);
        var request = $.ajax({
            url: $("#base_url").val() + "event/feedcomment?ajax=true" + values + "&id=" + $("#event_id_hidden").val(),
            type: "GET",
            data: {feed_id: values},
            dataType: "html"
        });
        ...
        request.done(function(msg) {
            ...
            $('#js_feed_comment_form_textarea_' + $data[0]).val('Write a comment...');
            if ($data[2] != '')
...
function ajax_like(type_id, item_id, feed_id) {
    ...
    var request = $.ajax({
        url: $("#base_url").val() + "feeds/like_feed" + "?id=" + $("#event_id_hidden").val(),
        type: "POST",
...
function ajax_Dislike(type_id, item_id, feed_id) {
    ...
    var request = $.ajax({
        url: $("#base_url").val() + "feeds/dislike_feed" + "?id=" + $("#event_id_hidden").val(),
        type: "POST",
        data: {type: type_id, item: item_id, feeds: feed_id},
        dataType: "html"
    });
...
//        alert(link);
    $(".activity_feed_form_share_process").css("display", "block");
    var request = $.ajax({
        url: $("#base_url").val() + "event/link?ajax=true" + "&id=" + $("#event_id_hidden").val(),
        type: "POST",
...
//            var old_start = c;
    ...
    var group_id = $("#group_id").val();
    var request = $.ajax({
        url: $("#base_url").val() + "event/display_feeds" + "?id=" + $("#event_id_hidden").val(),
        type: "POST",
