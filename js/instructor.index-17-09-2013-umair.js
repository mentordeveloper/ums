...
function add_reminder_event(ev_id) {
	open_reminder_ui(ev_id,'r_event_id');
}
...
function change_view_add() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#list_pop_types').html(data);
			$("#events_type_list").fancybox().trigger('click');
                        $('#datepicker12').timepicker();		
                        $('#datepicker21').timepicker();	
...
function selector_event(title,is_flag) {
	...
	var selected = $('#select_id_cal :selected').val();
        $('#name_test').val(selected)
	var hd_value = $('#hidden_event_type').val();
       // alert(selected);
        if(selected=='' || selected=='undefined'){
            selected=hd_value;
            $("#select_id_cal option[value='"+selected+"']").prop('selected', true);
        }
      //  alert("Selected : "+selected+" / Hidden"+hd_value);
        //var title = title.length == 0 ? $('#select_id_cal :selected').val() : title; 
    ...
    if ($('#select_id_cal :selected').text() == 'Assignment'
            || $('#select_id_cal :selected').text() == 'Reading' || $('#select_id_cal :selected').text() == 'Class'
            ) {
        $('.canremove').hide();
        $.ajax({
		...
		$('#error_type')
				.html(
						'<div class="alert alert-error"><button data-dismiss="alert" class="close"></button>									<strong>Error!</strong>'
								+ error + '</div>');
		$('#error_type').slideDown('slow', function() {
			$('#error_type').delay(2800).slideUp(1000);
		});
		$('#btn_save_action').attr('disabled', false);
		return false;
	}
	...
	if ($('#input_choice :selected').val() == 'textbox') {
		$('#event_typer_add')
				.append(
						'<tr id='
								+ id
								+ '><td><input class="m-wrap small" type="text" value="" /></td><td><input type="text" value="" /><br/><span onClick="removeitem('
								+ id
	...
	if ($('#input_choice :selected').val() == 'date_picker') {
		$('#event_typer_add')
				.append(
						'<tr id='
								+ id
								+ '><td><input class="m-wrap small" type="text" value="" /></td><td><input type="text" class="date_picker" /><br/><span onClick="removeitem('
								+ id
								+ ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
				.slideDown(500);
	...
	if ($('#input_choice :selected').val() == 'time_picker') {
		$('#event_typer_add')
				.append(
						'<tr id='
								+ id
								+ '><td><input class="m-wrap small" type="text" value="" /></td><td><input type="text" class="time_picker" /><br/><span onClick="removeitem('
								+ id
								+ ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
				.slideDown(500);
	...
	if ($('#input_choice :selected').val() == 'checkbox') {
		$('#event_typer_add')
				.append(
						'<tr id='
								+ id
								+ '><td><input class="m-wrap small" type="text" value="" /></td><td><input type="checkbox" /><br/><span onClick="removeitem('
								+ id
								+ ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
				.slideDown(500);
	...
	if ($('#input_choice :selected').val() == 'textarea') {
		$('#event_typer_add')
				.append(
						'<tr id='
								+ id
								+ '><td><input class="m-wrap small" type="text" value="" /></td><td><textarea cols="4" rows="5"></textarea><br/><span onClick="removeitem('
								+ id
								+ ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
				.slideDown(500);
	}
	...
	if ($('#input_choice :selected').val() == 'file_selector') {
		...
		$('#event_typer_add')
				.append(
						'<tr id='
								+ id
								+ '><td><input class="m-wrap small" type="text" value="" /></td><td><input type="file" name="uploaded_files[]" /><br/><span onClick="removeitem('
								+ id
								+ ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
				.slideDown(500);
...
function Delete_event(id) {
	...
	$.ajax({
		...
		type : "GET",
		data : 'id=' + id+'&course='+selected_text,
		context : document.body,
		cache : false,
		success : function(data) {
			...
			.fullCalendar(
					{
						...
						eventClick : function(event) {
                                                    ...
                                                    //  Qtipshow($(this), event['id']);
                                                        ...
                                                        if(textToFind=='Assignment' || textToFind=='Reading' || textToFind=='Class' ){
                                                            Qtipshow($(this), event['id']);
                                                        }
                                                        else{
                                                            $("#select_id_cal option[value='"+textToFind+"']").prop('selected', true);
                                                            var selected = $('#select_id_cal :selected').val();
                                                            Qtipshow_1($(this), event['id']);
                                                        }
                                                     //   alert(event['id']);
						...
						drop : function(date, allDay) { // this function is
							...
							$('#hidden_date').val(month+'-'+day+'-'+year);
...
function Save_event_go() {
	// //////////validations
        ...
        var typoo = $("#select_id_cal option:selected").val();
        if(typoo=='none'){
	...
	if (html != '') {
		...
		$('#errors_show').slideDown('slow', function() {
			$('#errors_show').delay(5000).slideUp(1300);
		});
                  after_loading();
		return false;
	} else {
		...
		var pass = true;
		var typecounter = 0;
if (typoo == 'Assignment' || typoo == 'Reading' || typoo == 'Class' ) {                
		$('.custom_options_id')
				.closest('tr')
				.find("input,textarea")
				.each(
...
//alert(datastring);
	...
	// function to display permissions to add events or not
...
function Update_event_go_front(id) {
    new_loading();
    if ($("#Filedata-queue > div").size() == 0) {
        Update_event_front(id);
    } else {
        $('#Filedata').uploadify('upload', '*');
    }
}

function Update_event_front(id) {

    $('#Filedata').uploadify('upload', '*');

    var html = '';

    var fields_type = $("input[name='form_fields_type']").val();
    var fields_name = $("input[name='form_fields_type']").attr("id");
    
    
    var fields_type_arr = fields_type.split(",");
    var fields_name_arr = fields_name.split(",");
    var typoo = $("#event_type_select").val()
    ...
    if (html != '') {
        ...
        var datastring = '';
        var notes = '';
        $i_index = $.inArray("Notes", fields_name_arr);
        if($i_index!="-1"){
            notes_id = fields_name_arr[$i_index];
        }
        else if($.inArray("Event_Notes", fields_name_arr)!="-1"){
            $i_index = $.inArray("Event_Notes", fields_name_arr);
            notes_id = fields_name_arr[$i_index];
        }
        else {
            $i_index = $.inArray("Event_Notes", fields_name_arr);
            notes_id = "note_calender";
        }
        ...
        if (typoo == 'Assignment' || typoo == 'Reading') {
            ...
            datastring += 'date=' + $('#datepicker2').val() + '&location='
                    + $("input[name='Location']").val() + '&starttime='
                    + $("#datepicker_front1").val() + '&end_date='
                    + $('#datepicker_front2').val() + "&type="
                    + typoo + '&notes='
                    + $("textarea[name='"+notes_id+"']").val() + '&id=' + id
                    + '&course=' + $("#course_id").val();

        }

        var custome_fields_data = '';
        var customdata = new Array();
        for(i_j=0;i_j<fields_name_arr.length;i_j++){
            if(fields_type_arr[i_j]=="textarea"){
              //  alert($("textarea[name='"+fields_name_arr[i_j]+"']").val());
                custome_fields_data += fields_name_arr[i_j]+'=' + $("textarea[name='"+fields_name_arr[i_j]+"']").val() + '&';
                customdata[i_j] = $("textarea[name='"+fields_name_arr[i_j]+"']").val();
            }else{
                custome_fields_data += fields_name_arr[i_j]+'=' + $("input[name='"+fields_name_arr[i_j]+"']").val() + '&';
                customdata[i_j] = $("input[name='"+fields_name_arr[i_j]+"']").val();
            }
 ...
 datastring +="&"+custome_fields_data+"field_name="+fields_name;
        ...
        var enoded_data         = JSON.stringify(customdata);
        var enoded_data_custom  = JSON.stringify(custome_fields_data);
        datastring += '&custom_type=' + fields_type_arr + '&custom_data=' + enoded_data;
        // ///custom types update code
//alert(datastring);
        $.ajax({
            url: $('#base_url').val() + "instructor/update_session_saved_front/",
            type: "GET",
            data: datastring,
            context: document.body,
            cache: false,
...
function Delete_event_front(id) {

    new_loading();
    alert("Are You Sure want to delete...");
    $.ajax({
        url: $('#base_url').val() + "instructor/delete_event_this_front/",
        type: "GET",
        data: 'id=' + id,
        context: document.body,
        cache: false,
...
function Qtipshow(obj, id) {
	obj.qtip({
				content : {
					...
					text : 'Loading.... ',
					ajax : {
						url : $('#base_url').val()+ "instructor/get_event_by_id/?id=" + id+"&event_date="+$('#hidden_date').val()
//                                                ,
//                                                async:false,
					},
					title : {
	...
	// now calling function which will change user UI according to selected event type which is dragged
        var textToFind = $('#hidden_event_type').val();
        $("#select_id_cal option[value='"+textToFind+"']").prop('selected', true);
	selector_event(textToFind,false);
...
function Qtipshow_title(obj, id,final_date) {
	...
	obj.qtip({
				content : {
					...
					ajax : {
						url : $('#base_url').val()
								+ "instructor/get_event_by_title/?id=" + id+"&event_date="+final_date,
                                                async :true ,                                                        
					},
					title : {
						text : 'Event Managment', // Give the tooltip a title
...
//					}
	...
	$("#select_id_cal option[value='"+id+"']").prop('selected', true);

	// now calling function which will change user UI according to selected event type which is dragged
	 selector_event(id,true);
...
function search_calendar_event() {
				...
				var myresult = $("#validate_search").validate({
										rules: {
											search_calendar: { 
												 required: true,
												},
										...
										messages: {
											search_calendar: "Please enter keyword.", 
										},
							            highlight: function (element) { // hightlight error inputs
						                  $(element).closest('.control-group').addClass('error'); // set error class to the control group
	            						},
...
function add_calendar(id) {
	...
	$('#inline1_email_confirm').html('<span class="success alert-success">calendar added successfully</span>');
     //   $('#inline1_email_confirm').css("display","none");
	$("#email_confirm_chooser").fancybox().trigger('click');
...
$(document).on('click', '#datepicker9', function ()  {$("#datepicker9").timepicker();});
$(document).on('click', '#datepicker10', function ()  {$("#datepicker10").timepicker();});
$(document).on('click', '#datepicker_front1', function ()  {$("#datepicker_front1").timepicker();});
$(document).on('click', '#datepicker_front2', function ()  {$("#datepicker_front2").timepicker();});
