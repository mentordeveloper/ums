...
function update_univ_info()
{
    $.ajax({
        ...
        success: function(data) {
            $('.msg').html('<div class="alert alert-success"><button class="close" data-dismiss="alert"></button><strong>Success! Updated Successfully</strong></div>');
        }
...
function save_univ_info()
{
    $.ajax({
        ...
        cache: false,
        success: function(data) {
            $('.msg').html('<div class="alert alert-success"><button class="close" data-dismiss="alert"></button><strong>Success! Added Successfully</strong></div>');
        }
...
function g_u_d() {
    ...
    var view = new google.picker.View(google.picker.ViewId.DOCS);
    var picker = new google.picker.PickerBuilder().enableFeature(google.picker.Feature.MULTISELECT_ENABLED).setAppId(
            "17296771986.apps.googleusercontent.com").setOAuthToken(
            "4/pAuvPJRATytJKaJNm-GzC8ycAk-y.0qn_1X_2ksUbOl05ti8ZT3Z1khhlfQI") // Optional:
			
            .addView(view).addView(new google.picker.DocsUploadView()).setCallback(g_uploadd).build();
		    picker.setVisible(true);
}
...
function g_uploadd(data) {
    ...
    if (data[google.picker.Response.ACTION] == google.picker.Action.PICKED) {
		...
		for(var docs=0;docs<=data.docs.length;docs++){
			
			 var url_each = data[google.picker.Response.DOCUMENTS][docs].url;
			
			 ajax_call_google_u_id(url_each);	
...
function ajax_call_google_u_id(my_url) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#notif_show').html('<div class="alert alert-success" style="margin-top:5px;"><button data-dismiss="alert" class="close"></button> File Attached! </div>');
...
function ajax_call_google_docs(id, event_id) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#notif_show').html('<div class="alert alert-success"><button data-dismiss="alert" class="close"></button><strong>Success:</strong> File Attached! </div>');
...
function add_reminder_event(ev_id) {
    ...
    $.ajax({
        url: $('#base_url').val() + "reminder/generate_new_ui_event/",
        type: "GET",
        data: "ev_id=" + ev_id,
        context: document.body,
        cache: false,
    ...
    $
        ...
        success: function(data) {
            ...
            if (data) {
                ...
                $('.error')
                        .html(
                        '<div class="alert alert-success">Color update successfully!</div>');
                $('#error_type').slideDown('slow', function() {
                    $('#error_type').delay(1800).slideUp(2000);
                });
                // //send ajax call and end this
...
function selector_event() {
    ...
    $('#select_id_cal').removeClass('error_events');
    // for generic information

    var selected = $('#select_id_cal :selected').val();
    if (selected == 'Assignment' || selected == 'Reading' || selected == 'School event') {
        $('.title-can-change').html('<b>Assigned date</b>');
    } else {
        $('.title-can-change').html('<b>Date</b>');
    }

    if ($('#select_id_cal :selected').text() == 'Assignment' || $('#select_id_cal :selected').text() == 'Reading' || $('#select_id_cal :selected').text() == 'School event') {
        $('.canremove').hide();
    } else {
                ...
                if (typoo != 'Assignment' && typoo != 'Reading') {
                    ...
                    if ($(this).attr('id') == 'type_name') {
                        ...
                        if ($(this).val() == '') {
                            ...
                            $("#type_name_error").fadeIn("fast");
                            $(this).focus();
                            $('#btn_update_action').attr('disabled', false);
                            type_error = true;
                            return false;
                        } else {
                            $("#type_name_error").fadeOut("fast");
                    ...
                    if ($(this).attr('id') == 'type_location') {
                        ...
                        if ($(this).val() == '') {
                            ...
                            $("#type_loc_error").fadeIn("fast");
                            $(this).focus();
                            $('#btn_update_action').attr('disabled', false);
                            type_error = true;
                            return false;
                        } else {
                            $("#type_loc_error").fadeOut("fast");
                            mydata[counter1] = $(this).val();
                        }
                    }
                    ...
                    if ($(this).attr('id') == 'type_name_starttime') {
                        ...
                        if ($(this).val() == '') {
                            ...
                            $("#type_starttime_error").fadeIn("fast");
                            $(this).focus();
                            $('#btn_update_action').attr('disabled', false);
                            type_error = true;
                            return false;
                        } else {
                            $("#type_starttime_error").fadeOut("fast");
                            mydata[counter1] = $(this).val();
                        }
                    }
                    ...
                    if ($(this).attr('id') == 'type_name_endtime') {
                        ...
                        if ($(this).val() == '') {
                            ...
                            $("#type_endtime_error").fadeIn("fast");
                            $(this).focus();
                            $('#btn_update_action').attr('disabled', false);
                            type_error = true;
                            return false;
                        } else {
                            $("#type_endtime_error").fadeOut("fast");
                            mydata[counter1] = $(this).val();
                        }
                    }
...
// to get date time picker type just add class on the other hand
            ...
            else if ($(this).attr('class') == 'date_picker m-wrap hasDatepicker') {
                types[typecounter] = 'date_picker';
            } // to get time picker type just add class on the other hand
            else if ($(this).attr('class') == 'time_picker m-wrap hasDatepicker') {
                types[typecounter] = 'time_picker';
            } 
            else {
...
function show_permissions_for_cal()
{
    $('#selected_event').html('');
    flag = 0;
    ...
    if ($('#core_permission').val() == 'view')
    {
        ...
        if (text_selected == 'School Calendar')
        {
            flag = 1;
        } 
        else if (text_selected == 'all' || text_selected == 'All')
        {
            flag = 0;
        }
        else if (text_selected != 'Personal')
        {
// this ajax call is to check if user has rights to add events to selected course ..as it can be its own added calender
            $.ajax({
                url: $('#base_url').val() + "instructor/check_if_rights_of_course/",
                type: "GET",
                ...
                success: function(data) {
                    ...
                    if (flag == 1)
                    {
                        $('#selected_event').html('<div style="margin-bottom:0px; !important;width:300px" class="alert alert-info"><button class="close" data-dismiss="alert"></button>You cannot add events in this selected calendar! </strong></div>');
                        $("#added_event").fancybox().trigger('click');
                        setTimeout(function() {
                            $.fancybox.close();
                        }, 2000);
                        return 0;
    ...
    else if ($('#core_permission').val() == 'modify')
    {
        if (text_selected == 'School Calendar' && $('#canmanage_school_calender').val() != 'admin school')
        {
            flag = 1;
        } else if (text_selected == 'all' || text_selected == 'All')
        {
            ...
            function() {
                ...
                if (pass) {
                    ...
                    if ($(this).val() == '') {
                        ...
                        $("#type_custom_error_" + tr_id).fadeIn("fast");
                        $(this).focus().scrollTop();
                        $('#btn_save_action').attr('disabled', false);
                        type_error = true;
                        return false;
                    } else {
                        $("#type_custom_error_" + tr_id).fadeOut("fast");
                        mydata[counter1] = $(this).val();
                    }
...
// to get date time picker type just add class on the other hand
    ...
    if (error != '' && !type_error) {
        $('#error_type')
                ...
                + error + '</div>');
        $('#error_type').slideDown('slow', function() {
            $('#error_type').delay(280000).slideUp(100000);
        });
        $('#btn_save_action').attr('disabled', false);
        return false;
    }
...
function new_field_add() {
    ...
    var custom_fields_add = $("#custom_fields_add").val();
    if ($('#input_choice :selected').val() == 'textbox') {
        $('#event_typer_add')
                .append(
                '<tr class="canremove_1" id='
                + id
                + '><td><input class="m-wrap small " type="text" value="" placeholder="Enter Field Name" /></td><td><input id="txt_' + id + '" class="m-wrap " type="text" value="" placeholder="Enter Text Here..." /><span style="float: left; color:#900;display: none;" id="type_custom_error_' + id + '">Please enter text ...!</span><br/><span onClick="removeitem('
                + id
                + ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
                .slideDown(500);
        custom_fields_add += ", txt_" + id;
        $("#custom_fields_add").val(custom_fields_add);
    }
    ...
    if ($('#input_choice :selected').val() == 'date_picker') {
        $('#event_typer_add')
                ...
                '<tr class="canremove_1" id='
                + id
                + '><td><input class="m-wrap small " type="text" value=""  placeholder="Enter Field Name" /></td><td><input id="txt_' + id + '" placeholder="Select Date..."  type="text" class="date_picker m-wrap" /><span style="float: left; color:#900;display: none;" id="type_custom_error_' + id + '">Please Select Date ...!</span><br/><span onClick="removeitem('
                + id
                + ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
                .slideDown(500);
    ...
    if ($('#input_choice :selected').val() == 'time_picker') {
        $('#event_typer_add')
                ...
                '<tr class="canremove_1" id='
                + id
                + '><td><input class="m-wrap small " type="text" value="" placeholder="Enter Field Name" /></td><td><input type="text" id="txt_' + id + '" placeholder="Select Time..."  class="time_picker m-wrap" /><span style="float: left; color:#900;display: none;" id="type_custom_error_' + id + '">Please Select Time ...!</span><br/><span onClick="removeitem('
                + id
                + ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
                .slideDown(500);
    ...
    if ($('#input_choice :selected').val() == 'checkbox') {
        $('#event_typer_add')
                ...
                '<tr class="canremove_1" id='
                + id
                + '><td><input class="m-wrap small" type="text" value="" placeholder="Enter Field Name" /></td><td><input class="m-wrap " id="txt_' + id + '" placeholder="Enter Text Here..." type="checkbox" /><span style="float: left; color:#900;display: none;" id="type_custom_error_' + id + '">Please Select CheckBox ...!</span><br/><span onClick="removeitem('
                + id
                + ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
                .slideDown(500);
        $('.time_picker').timepicker();
    ...
    if ($('#input_choice :selected').val() == 'textarea') {
        $('#event_typer_add')
                ...
                '<tr class="canremove_1" id='
                + id
                + '><td><input class="m-wrap small" type="text" value=""  placeholder="Enter Field Name" /></td><td><textarea id="txt_' + id + '" class="m-wrap " cols="4" rows="5"></textarea><span style="float: left; color:#900;display: none;" id="type_custom_error_' + id + '">Please enter text ...!</span><br/><span onClick="removeitem('
                + id
                + ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
                .slideDown(500);
        custom_fields_add += ", txt_" + id;
    ...
    if ($('#input_choice :selected').val() == 'file_selector') {
        ...
        $('#event_typer_add')
                ...
                '<tr class="canremove_1" id='
                + id
                + '><td><input class="m-wrap small" type="text" value="" placeholder="Enter Field Name" /></td><td><input class="m-wrap " id="txt_' + id + '" type="file" name="uploaded_files[]" /><span style="float: left; color:#900;display: none;" id="type_custom_error_' + id + '">Please Select File ...!</span><br/><span onClick="removeitem('
                + id
                + ')" style="cursor:pointer; color:#900"><b>remove</b></span></td></tr>')
                .slideDown(500);
        custom_fields_add += ", txt_" + id;
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (obj['error'] == 1) {
                ...
                $('#updating_event_html_overall').html(obj['html']);
                refresh_calender_on_call();
            }
        }
    });
...
// notification
        ...
        $('#inline1_email_confirm')
                .html(
                '<div class="alert alert-success"><button data-dismiss="alert" class="close"></button><strong>Success!</strong> Notifications Sended! </div>');
...
function Delete_event(id) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            /*
             * if removing from the event then remive the reminder itself
             */

            if (obj['response'] == 1)
            {
                confirm_remove(obj['id']);
            }

            $('#msg_user')
                    .html(
                    '<div class="alert alert-success" style="margin-bottom:0px;width:200px;"><button data-dismiss="alert" class="close"></button><strong>Success!</strong> Removed! </div>');
...
// if conditions for check of assignmnts and class events not visible to student
    if ($('#canmanage_school_calender').val() == 'student' && title == 'Assignment')
    {
    } else if ($('#canmanage_school_calender').val() == 'student' && title == 'Class')
    {
...
// alert("Event_id:"+event['id']);alert("Event_start:"+event['start']);

//   alert(event['id']);


                    var date_target = new Date(event.start);
                    var year = parseInt(date_target.getFullYear());
                    var month = parseInt(date_target.getMonth() + 1);
                    var day = parseInt(date_target.getDate());
                    var final_date = month + '-' + day + '-' + year;
                    Qtipshow($(this), event['id'], final_date, encodeURIComponent(event.title));
                ...
                eventDrop: function(event, dayDelta, minuteDelta, allDay, revertFunc) {
                    ...
                    $.ajax({
                        url: $('#base_url').val() + "instructor/event_remap/",
                        type: "GET",
                        data: 'eid=' + event.id + '&esdate='
                                + event.start + '&enday=' + dayDelta
                                + '&emin=' + minuteDelta,
                        context: document.body,
                        cache: false,
                        success: function(data) {
                            ...
                            if (obj['response'] == 0)
                            {
                                $('#msg_user')
                                        .html(
                                        '<div style="margin-bottom:0px;" class="alert alert-error"><button data-dismiss="alert" class="close"></button><strong>Error! </strong>' + obj['msg'] + '</div>');
                                $("#global_use_msg").fancybox().trigger('click');
                                setTimeout(function() {
                                    $.fancybox.close();
                                }, 2000);
                            ...
                            if (obj['response'] == 1)
                            {
                                ...
                                $('#msg_user')
                                        .html(
                                        '<div style="margin-bottom:0px;" class="alert alert-success"><button data-dismiss="alert" class="close"></button><strong>Success! </strong>' + obj['msg'] + '</div>');
                                $("#global_use_msg").fancybox().trigger('click');
                                setTimeout(function() {
                                    $.fancybox.close();
                                }, 1000);
                ...
                drop: function(date, allDay) {
                    ...
                    /*
                     * is dropped from left side container
                     */

                    validate_some_courses_alert();


                    var obj = $(this).data('eventObject');
                    $('#hidden_event_type').val(obj['title']);
                    $('#save_date').val(date);
                    var date_target = new Date(date);
                    var year = parseInt(date_target.getFullYear());
                    var month = parseInt(date_target.getMonth() + 1);
                    var day = parseInt(date_target.getDate());
                    var final_date = month + '-' + day + '-' + year;
                    $('#hidden_date').val(final_date);
                    ...
                    //Qtipshow_title($(this), obj['title'],final_date);
...
//alert(event.id+" : Start Date : "+event.start+" : The end date of " + event.title + "has been moved " +dayDelta + " days and " + minuteDelta + " minutes."        );
                    if (!confirm("Are you sure?")) {
                        revertFunc();
                    } else {
                        $.ajax({
                            url: $('#base_url').val() + "instructor/update_event_session/",
                            type: "GET",
                            data: 'eid=' + event.id + '&esdate='
                                    + event.start + '&enday=' + dayDelta
                                    + '&emin=' + minuteDelta,
...
//alert(data);
                ...
                select: function(start, end, allDay) {
                    ...
                    /*
                     * 
                     * some checks to make them alert
                     * 
                     */
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            var tmp_json = '{"id":"none","title":"New Years Day","allday":"true","allDay":"true","start":'
                                    ...
                                    + ',"end":'
                                    + end
                                    + ',"ev_assigned_date":"1-1-2013","className":"holidays","editable":true}';
                            calender_render();
                        }
                    });
                },
...
function validate_some_courses_alert(course)
{
    /*
     * 
     * fix some courses so easy for users to think for them in mapping events
     * 
     */

    var course = $("#field_courses option:selected").text();
    var user_role = $('#login_type').val();
//    if (course == 'All' || course == 'School Calendar' && user_role != 'admin school')
    if (course == 'School Calendar' && user_role != 'admin school')
    {

        $('#selected_event').html('<div style="margin-bottom:0px; !important;width:300px" class="alert alert-info"><button class="close" data-dismiss="alert"></button>Please select course !</strong></div>');
        $("#added_event").fancybox().trigger('click');
        setTimeout(function() {
            $.fancybox.close();
        }, 1300);
...
// /////make sure that for new event allday is set to disabled
    ...
    if (typoo != 'Assignment' && typoo != 'Reading' && typoo != 'School event') {
        ...
        if ($('#location_calender').val() == '') {
            ...
            $('#select_id_cal').addClass('error_events');
            $('#functions').show();
            $('#loading').hide();
            return;
        } else {
    ...
    if (typoo == 'Assignment' || typoo == 'Reading') {
        ...
        if (end >= start) {
            ...
            /*
             * if assigned date is greater then due date
             */

            $('#datepicker2').addClass('error_events');
            $('#datepicker3').addClass('error_events');
            $('#functions').show();
            $('#loading').hide();
            $('#viewer').append('<div class="alert alert-error"><button data-dismiss="alert" class="close"></button><strong>Error!</strong> Date Due must be greater!.</div>');
...
// /if all day is selected
    ...
    if (html != '') {
        ...
        var typecounter = 0;
        /*
         *  loop thorugh all form and save its values
         */
        ...
        $('.custom_options_id')
                ...
                function() {
                    ...
                    if ($(this).attr('class') != 'custom_options_id_not' && $(this).attr('type') != 'button') {

                        if ($(this).attr('required')) {

                            if ($(this).val() == '') {
                                $(this).addClass("error_events");
                                $('#functions').show();
                                $('#loading').hide();
                                pass = false;
                            }
...
function show_event(id, event_done) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (obj[0]['invite_status'] != 'no' && obj[0]['invite_status'] != '') {
...
// /hide the prevois displayer we dont need it anymore
            ...
            if (view_checker) {
                ...
                var start_time = $.trim(obj[0]['ev_start_time']);
                var end_time = $.trim(obj[0]['ev_end_time']);
                ...
                if (!start_time && !end_time) {
                    ...
                    $('#datepicker4_v1').val(obj[0]['ev_start_time']);
                    $('#datepicker6_v1').val(obj[0]['ev_end_time']);
...
// ///////////////////custom fields code here
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#selected_event').html('<div style="margin-bottom:0px; !important;width:300px" class="alert alert-info"><button class="close" data-dismiss="alert"></button>Updated Successfully!</strong></div>');
                $("#added_event").fancybox().trigger('click');
                setTimeout(function() {
                    $.fancybox.close();
                }, 2000);
...
function Qtipshow(obj, id, date_selected, type_name) {
    ...
    obj.qtip({
        content: {
            ...
            ajax: {
                ...
                data: {
                    "id": id,
                    "event_date": date_selected,
                    "type_name": type_name,
                    "sv_1": selected_name,
                },
            },
            title: {
                text: selected_name + ' Management',
            }
        },
        position: {
            target: 'rightMiddle',
    ...
    obj.qtip("show");
//
//    // if its only dragged event then we need alert
//    if ($('#hidden_date').val() != "")
//        alert("Date Selected  : " + $('#hidden_date').val());

    //making date empty as if he selects any other event so it will not remain in session..we are making it empty after using
//    $('#hidden_date').val('');

//    // selected event type that is dragged 
//    var textToFind = $('#hidden_event_type').val();
//
//    // assigning selected event type to dropdown as selected
//    var dd = document.getElementById('select_id_cal');
//    if (textToFind != "" && dd != null)
//    {
...
//            }
//        }
//        // now calling function which will change user UI according to selected event type which is dragged
//        selector_event();
//
//        // now making selected event type empty so that it will not use again if we select anyother event
//        $('#hidden_event_type').val('');
...
// notification
        ...
        var success = 0;
        if (return_data.length == 0) {
            $('#notifications_send').html('<label class="alert alert-error">please select atleast one user</label>');
            return false;
        }
        for (var k = 0; k < return_data.length; k++) {
        ...
        $('#inline1_email_confirm').html('<div style="margin-bottom:0px; !important;width:200px" class="alert alert-success"><button class="close" data-dismiss="alert"></button><strong>Success!</strong>Event Shared </div>');
        $("#email_confirm_chooser").fancybox().trigger('click');
        setTimeout(function() {
            $.fancybox.close();
        }, 1800);
...
function accept_share_event(event_id, share_id) {
    ...
    $('#inline1_email_confirm').html('<div style="margin-bottom:0px;" class="alert alert-success"><button data-dismiss="alert" class="close"></button><strong>Event accepted!</strong> </div>');
    $("#email_confirm_chooser").fancybox().trigger('click');
...
function deny_share_event(event_id, share_id) {
    ...
    $('#inline1_email_confirm').html('<div style="margin-bottom:0px;" class="alert alert-success"><button data-dismiss="alert" class="close"></button><strong>Event Denied!</strong> </div>');
    $("#email_confirm_chooser").fancybox().trigger('click');
...
function add_calendar(id) {
    ...
    $('#msg_user').html('<div style="margin-bottom:0px;" class="alert alert-success"><button data-dismiss="alert" class="close"></button><strong>Calendar added successfully!</strong> </div>');
    $("#global_use_msg").fancybox().trigger('click');
...
$(function() {
    $("#type_name_starttime").timepicker();
    $("#type_name_endtime").timepicker();
    $("#datepicker9").timepicker();
    $("#datepicker10").timepicker();
    $('#datepicker3').timepicker();
    $('#datepicker4').timepicker();
...
$(document).on('click', '#datepicker9', function() {
    $("#datepicker9").timepicker();
});
$(document).on('click', '#datepicker10', function() {
    $("#datepicker10").timepicker();
});
$(document).on('click', '#datepicker_front1', function() {
    $("#datepicker_front1").timepicker();
});
$(document).on('click', '#datepicker_front2', function() {
    $("#datepicker_front2").timepicker();
});

function view_all_user_list() {
    var value = $("#field_courses option:selected").val();
    var value_text = $("#field_courses option:selected").text();
   
    $.ajax({
        url: $('#base_url').val() + "instructor/permission_view_list/",
        type: "POST",
        data:{sel_val:value,sel_txt:value_text},
        context: document.body,
        cache: false,
        success: function(data) {
...
function permission_assing_course(a,b){
    $("#fancybox-loading").css('display','block');
    var s = 0;
    if($("#permission_"+b).is(':checked'))
          s = 1;
    console.log(s);
    $.ajax({
        url: $('#base_url').val() + "instructor/update_user_course_permission_list/",
        type: "POST",
        data:{id:a,u:b,st:s},
        context: document.body,
        cache: false,
